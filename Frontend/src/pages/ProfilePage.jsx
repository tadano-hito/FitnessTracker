...
import {
  User, Camera, Save, LayoutDashboard, Droplet,
...
export default function ProfilePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('')
    try {
      const { data } = await axios.put('http://localhost:5003/api/auth/profile',
        form,
        { headers: { Authorization: `Bearer ${token}` } }
      )
      login(data, token)
    ...
    <div className="bg-black min-h-screen flex text-white">
      ...
      <main className={`${sidebarOpen ? 'ml-64' : 'ml-16'} transition-all duration-300 flex-1 p-4 md:p-8 min-w-0`}>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-1 flex flex-col gap-4">
              ...
              <div className="relative mb-4">
                <div className="w-28 h-28 rounded-full border-2 border-green-400/30 overflow-hidden bg-white/5 flex items-center justify-center">
                  {previewUrl || profilePicture ? (
                    <img src={previewUrl || profilePicture} alt="Profile"
            ...
            {bmi && (
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6"
                style={{ borderColor: `${bmiStatus?.color}30`, background: `${bmiStatus?.color}08` }}>
                ...
                <p className="text-sm font-bold uppercase tracking-wider mb-4" style={{ color: bmiStatus?.color }}>
                  {bmiStatus?.label}
                </p>
                {/* BMI bar */}
            ...
            {(form.weight || form.height || form.age) && (
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  {form.age && (
                    <div>
                      <p className="text-2xl font-black text-green-400">{form.age}</p>
                      <p className="text-xs text-gray-500">Age</p>
                    </div>
                  )}
                  {form.weight && (
                    <div>
                      <p className="text-2xl font-black text-green-400">{form.weight}</p>
                      <p className="text-xs text-gray-500">kg</p>
                    </div>
                  )}
                  {form.height && (
                    <div>
                      <p className="text-2xl font-black text-green-400">{form.height}</p>
          ...
          {/* Right — Edit form */}
          <div className="xl:col-span-2 bg-white/5 border border-white/10 rounded-2xl p-6 md:p-8">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                      ...
                      <button key={g} type="button"
                        ...
                        className={`py-3 rounded-xl text-xs font-bold uppercase transition border
                          ${form.gender === g
                            ...
                            : 'border-white/10 text-gray-500 hover:text-white bg-white/5'}`}>
                        {g}
                      </button>
                    ))}
                  </div>
              ...
              {form.weight && form.height && (
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl border"
                  ...
                  <p className="text-sm font-bold" style={{ color: bmiStatus?.color }}>
