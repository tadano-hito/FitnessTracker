...
import { Droplet, Moon, HeartPulse, Flame, Plus } from 'lucide-react'
...
const calculateStreak = (logs) => {
  ...
  for (const date of dates) {
    if (date === current || date === current - 86400000) { streak++; current = date - 86400000 }
    else break
  }
  return streak
}
...
export default function Dashboard() {
  ...
  ;[...sleepLogs].reverse().forEach(l => {
    ...
    if (!sleepByDay[dateKey]) sleepByDay[dateKey] = { day: dayLabel, hours: l.hoursSlept, quality: l.quality }
  })
  ...
  const bpChartData = [...bpLogs].reverse().slice(-5).map(l => ({
    ...
    systolic: l.systolic, diastolic: l.diastolic
  }))
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
          <p className="text-gray-500 text-xs uppercase tracking-widest mb-1">{greeting.text} {greeting.icon}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="rounded-2xl p-5 flex items-center gap-5 border"
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Based on water, sleep & BP</p>
          </div>
        </div>
            ...
            {[
              ...
              return (
                <div key={g.label}>
                  ...
                  <div className={`w-full ${darkMode ? 'bg-white/5' : 'bg-gray-100'} rounded-full h-1.5`}>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 md:gap-4 mb-6">
        <div className={`col-span-2 md:col-span-1 ${card} p-4 flex flex-col items-center hover:border-green-500/30 transition`}>
          <svg width="100" height="100" className="mb-2">
            <circle cx="50" cy="50" r={r} fill="none" stroke="#ffffff08" strokeWidth="8" />
            <circle cx="50" cy="50" r={r} fill="none"
              stroke={waterPct >= 30 ? '#4ade80' : '#f87171'}
              strokeWidth="8" strokeLinecap="round"
              strokeDasharray={`${waterDash} ${circ}`}
              strokeDashoffset={circ * 0.25}
              style={{ transition: 'stroke-dasharray 0.8s ease', filter: 'drop-shadow(0 0 8px #4ade80)' }}
            />
            <text x="50" y="46" textAnchor="middle" fill={darkMode ? '#fff' : '#111'} fontSize="13" fontWeight="bold">{waterTotal}ml</text>
          ...
          <span className="text-xs font-semibold text-gray-500 mb-2">Water Intake</span>
        ...
        <div className={`${card} p-4 hover:border-green-500/30 transition`}>
          ...
          {lastSleep ? (
            <>
              ...
              <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: getSleepColor(lastSleep.hoursSlept) }}>{lastSleep.quality}</p>
            </>
          ) : <p className="text-3xl font-black text-gray-400 mb-4">--</p>}
        ...
        <div className={`${card} p-4 hover:border-green-500/30 transition`}>
          ...
          {lastBP ? (
            <>
              ...
              <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: bpStatus?.color }}>{bpStatus?.label}</p>
            </>
          ) : <p className="text-3xl font-black text-gray-400 mb-4">--/--</p>}
        ...
        <div className={`${card} p-4 hover:border-orange-500/30 transition`}>
          <div className="flex items-center gap-1.5 mb-3">
            <Flame size={13} className="text-orange-400" />
            <span className="text-xs font-semibold text-gray-500">Streak</span>
          </div>
          <p className="text-3xl font-black mb-0.5">{streak}<span className="text-sm text-gray-500 font-normal"> days</span></p>
          <p className="text-xs font-bold uppercase tracking-wider mb-3 text-orange-400">{streak > 0 ? '🔥 Keep it up!' : 'Start logging!'}</p>
          <p className="text-xs text-gray-500">Log daily to grow</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div className={`${card} p-5`}>
          ...
          {sleepChartData.length === 0 ? (
            <div className="flex items-center justify-center h-32 text-gray-400 text-xs">No sleep data yet</div>
          ) : (
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={sleepChartData} barSize={24}>
                ...
                <Tooltip
                  ...
                  formatter={(v, n, props) => [`${v} hrs — ${props?.payload?.quality || ''}`, 'Sleep']}
                />
                <Bar dataKey="hours" radius={[6, 6, 0, 0]} fill="#4ade80" label={false} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className={`${card} p-5`}>
          ...
          {bpChartData.length === 0 ? (
            <div className="flex items-center justify-center h-32 text-gray-400 text-xs">No BP data yet</div>
          ) : (
            <ResponsiveContainer width="100%" height={160}>
              <LineChart data={bpChartData}>
                ...
                <Tooltip
                  contentStyle={{ background: darkMode ? '#111' : '#fff', border: '1px solid #ffffff15', borderRadius: '12px', color: darkMode ? '#fff' : '#111' }}
